...
from benchmark import job_setup, batch_submit  #, cleanup
import numpy as np
import os
import pandas as pd
...
def bench_data_setup(args):
    ...
    job_setup.setup_pdbs_folder(args.benchmark_dir)
...
def bench_from_step1(args):
    ...
    # Should not happend if 'data_setup' sub-command was used:
    if not args.benchmark_dir.joinpath(BENCH.CLEAN_PDBS).exists():
        #-> log.exceptions
        raise FileNotFoundError()

    sh_path = job_setup.write_run_script(args.benchmark_dir,
                                         args.job_name,
                                         #steps_options_dict,
                                         #sh_template
                                         )
...
def bench_parser():
    ...
    sub0 = subparsers.add_parser(SUB_CMD1,
                                 formatter_class = RawDescriptionHelpFormatter,
                                 help=HELP_1)
    sub1.add_argument(
        "-benchmark_dir",
